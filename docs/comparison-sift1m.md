...
| id | workload | sm-sift-perf-0926 qps | qd-sift-perf-0926 qps | ratio | sm-sift-perf-0926 p50/p99 | qd-sift-perf-0926 p50/p99 | recall@10 | notes |
|---|---|--:|--:|--:|--:|--:|--:|---|
| W0 | d=4 floor: graph traversal with the distance taken out | 4,348 | 3,458 | 1.26x | 225 µs / 274 µs | 289 µs / 362 µs | - | 1.26x is 1.71x less work per query x 0.68x cores busy during the row (0.69 against 1.01) x 1.00x clock x 1.09x counted on-CPU share: the middle term is occupancy, not search speed |
| W3 | search, fp32, single query | 1,699 | 2,019 | 0.84x | 600 µs / 714 µs | 500 µs / 614 µs | 0.9887 / 0.9874 |  |
| W4 | search, saturating (closed loop) | 23,166 | 14,075 | 1.65x | 2.76 ms / 3.06 ms | 4.04 ms / 12.72 ms | 0.9887 / 0.9874 |  |
| W4-sat50 | search, fixed rate at 50% of saturation (open loop) | 5,250 | 5,250 | offered | 505 µs / 1.32 ms | 682 µs / 1.34 ms | 0.9887 / 0.9874 |  |
| W4-sat70 | search, fixed rate at 70% of saturation (open loop) | 7,351 | 7,351 | offered | 481 µs / 907 µs | 759 µs / 1.85 ms | 0.9887 / 0.9874 |  |
| W4-sat90 | search, fixed rate at 90% of saturation (open loop) | 9,450 | 9,450 | offered | 523 µs / 1.13 ms | 814 µs / 1.93 ms | 0.9887 / 0.9874 |  |
| W5 | search batched (16 distinct dataset queries per request) | 23,463 | 4,845 | 4.84x | 1.36 ms / 1.51 ms | 6.59 ms / 7.25 ms | 0.9887 / 0.9874 | per-batch latency (16 queries/request); queries: dataset, random-sample 4.84x is 1.33x less work per query x 3.59x cores busy during the row (7.05 against 1.97) x 1.00x clock x 1.01x counted on-CPU share: the middle term is occupancy, not search speed |
| W6 | quantized: scalar | 4,219 | 4,674 | parity | 462 µs / 651 µs | 427 µs / 510 µs | 0.9892 / 0.9875 | within the ±20.3% band this dataset's noise floor puts on W6: no measured difference, not a small one |
| W6-ef32 | SQ8 recall control, ef=32 (latency only) | 8,857 | 7,853 | - | 223 µs / 280 µs | 248 µs / 327 µs | 0.9094 / 0.8984 | recall unequal: 0.9094 vs 0.8984; §7.4 compares at equal recall; the rescore pools match, so the encoders differ |
| W6-ef64 | SQ8 recall control, ef=64 (latency only) | 6,020 | 6,234 | parity | 334 µs / 434 µs | 312 µs / 417 µs | 0.9646 / 0.9600 | within the ±9.8% band this dataset's noise floor puts on W6-ef64: no measured difference, not a small one |
| W6-ef128 | SQ8 recall control, ef=128 (latency only) | 4,204 | 4,678 | parity | 463 µs / 650 µs | 427 µs / 504 µs | 0.9892 / 0.9875 | within the ±20.3% band this dataset's noise floor puts on W6-ef128: no measured difference, not a small one |
| W6-ef256 | SQ8 recall control, ef=256 (latency only) | 2,924 | 3,064 | 0.95x | 698 µs / 837 µs | 660 µs / 757 µs | 0.9974 / 0.9968 |  |
| W6-ef512 | SQ8 recall control, ef=512 (latency only) | 1,727 | 1,822 | 0.95x | 1.19 ms / 1.41 ms | 1.12 ms / 1.28 ms | 0.9995 / 0.9993 |  |
| W7 | quantized: binary + oversampling | 3,541 | 4,637 | - | 549 µs / 736 µs | 428 µs / 517 µs | 0.0605 / 0.0659 | recall below the usable floor: 0.0605 and 0.0659, both under 0.50 — this row characterises the encoding, not the engines |
| W8 | quantized: PQ | 3,060 | 4,164 | 0.73x | 672 µs / 806 µs | 472 µs / 609 µs | 0.9829 / 0.9781 |  |
| W9 | exact / brute force | 184 | 122 | 1.50x | 38.47 ms / 70.02 ms | 65.24 ms / 79.09 ms | - |  |
| W10-ef32 | recall control, ef=32 (latency only) | 40,330 | 21,915 | - | 190 µs / 255 µs | 336 µs / 657 µs | 0.9089 / 0.8982 | sm-sift-perf-0926: -n 100000 (2x the table's 50000) recall unequal: 0.9089 vs 0.8982; §7.4 compares at equal recall |
| W10-ef64 | recall control, ef=64 (latency only) | 31,261 | 15,617 | 2.00x | 252 µs / 316 µs | 473 µs / 956 µs | 0.9642 / 0.9605 | sm-sift-perf-0926: -n 100000 (2x the table's 50000) |
| W10-ef128 | recall control, ef=128 (latency only) | 20,641 | 10,203 | 2.02x | 389 µs / 481 µs | 711 µs / 1.50 ms | 0.9887 / 0.9874 |  |
| W10-ef256 | recall control, ef=256 (latency only) | 12,139 | 6,567 | 1.85x | 665 µs / 884 µs | 1.09 ms / 2.32 ms | 0.9974 / 0.9969 |  |
| W10-ef512 | recall control, ef=512 (latency only) | 6,659 | 3,946 | 1.69x | 1.21 ms / 1.71 ms | 1.86 ms / 3.75 ms | 0.9995 / 0.9993 |  |
| W12-sel1 | filtered search, one keyword (~1% of bench12) | 5,325 | 3,852 | 1.38x | 342 µs / 492 µs | 514 µs / 601 µs | 1.0000 |  |
| W12-sel10 | filtered search, any of 10 keywords (~10% of bench12) | 1,279 | 2,214 | - | 1.60 ms / 1.81 ms | 907 µs / 1.08 ms | 0.9993 / 0.9892 | recall unequal: 0.9993 vs 0.9892; §7.4 compares at equal recall |
| W12-sel1-ef32 | filtered recall control, one keyword, ef=32 (latency only) | 6,007 | 6,438 | parity | 316 µs / 472 µs | 304 µs / 397 µs | 1.0000 / 0.9964 | within the ±20.3% band this dataset's noise floor puts on W12-sel1-ef32: no measured difference, not a small one |
| W12-sel1-ef64 | filtered recall control, one keyword, ef=64 (latency only) | 6,114 | 5,255 | - | 309 µs / 468 µs | 376 µs / 445 µs | 1.0000 / 0.9997 | sm-sift-perf-0926 -11% across its passes, monotonically: the spread on this row is drift rather than noise, so the median is a trend's midpoint and not a repeatable measurement |
| W12-sel1-ef128 | filtered recall control, one keyword, ef=128 (latency only) | 5,909 | 3,850 | - | 316 µs / 477 µs | 514 µs / 604 µs | 1.0000 | sm-sift-perf-0926 -10% across its passes, monotonically: the spread on this row is drift rather than noise, so the median is a trend's midpoint and not a repeatable measurement |
| W12-sel1-ef256 | filtered recall control, one keyword, ef=256 (latency only) | 6,101 | 2,679 | 2.28x | 311 µs / 463 µs | 742 µs / 831 µs | 1.0000 |  |
| W12-sel1-ef512 | filtered recall control, one keyword, ef=512 (latency only) | 6,395 | 1,706 | 3.75x | 300 µs / 452 µs | 1.17 ms / 1.32 ms | 1.0000 | 3.75x is 4.87x less work per query x 0.80x cores busy during the row (1.60 against 2.00) x 1.00x clock x 0.97x counted on-CPU share: the middle term is occupancy, not search speed |
| W12-sel10-ef32 | filtered recall control, any of 10 keywords, ef=32 (latency only) | 3,155 | 4,658 | - | 630 µs / 871 µs | 424 µs / 542 µs | 0.9778 / 0.7980 | recall unequal: 0.9778 vs 0.7980; §7.4 compares at equal recall |
| W12-sel10-ef64 | filtered recall control, any of 10 keywords, ef=64 (latency only) | 2,065 | 3,367 | - | 989 µs / 1.11 ms | 593 µs / 722 µs | 0.9954 / 0.9326 | recall unequal: 0.9954 vs 0.9326; §7.4 compares at equal recall |
| W12-sel10-ef128 | filtered recall control, any of 10 keywords, ef=128 (latency only) | 1,269 | 2,205 | - | 1.61 ms / 1.84 ms | 911 µs / 1.09 ms | 0.9993 / 0.9892 | recall unequal: 0.9993 vs 0.9892; §7.4 compares at equal recall |
| W12-sel10-ef256 | filtered recall control, any of 10 keywords, ef=256 (latency only) | 781 | 1,317 | 0.59x | 2.60 ms / 3.02 ms | 1.53 ms / 1.83 ms | 0.9998 / 0.9991 |  |
| W12-sel10-ef512 | filtered recall control, any of 10 keywords, ef=512 (latency only) | 1,344 | 799 | 1.68x | 1.50 ms / 1.70 ms | 2.51 ms / 3.04 ms | 1.0000 |  |
| W13 | scroll / pagination | 50,266 | 46,002 | 1.09x | 139 µs / 235 µs | 160 µs / 291 µs | - | -n 100000 (2x the table's 50000) sm-sift-perf-0926: 0.13 ms of the client's 0.14 ms p50 is not server time (13x), so 92% of this row is the load generator and the socket qd-sift-perf-0926: 0.13 ms of the client's 0.16 ms p50 is not server time (6x), so 82% of this row is the load generator and the socket 1.09x is 3.27x less work per query x 0.33x cores busy during the row (1.39 against 4.21) x 1.00x clock x 1.02x counted on-CPU share: the middle term is occupancy, not search speed |
| W11-steady | mixed read/write below the rebuild threshold: search bench2 while 50,000 synthetic points append | 18,188 | 5,474 | - | 438 µs / 560 µs | 1.27 ms / 4.72 ms | - | sm-sift-perf-0926: search covered 11% of the append; append 2,000 points/s qd-sift-perf-0926: search covered 37% of the append; append 2,000 points/s search-during-write; no recall join |
| W11 | mixed read/write: search bench2 while 200,000 synthetic points append (runs last) | 1,940 | 2,294 | - | 3.40 ms / 10.07 ms | 2.72 ms / 14.88 ms | - | sm-sift-perf-0926: search covered 43% of the append; append 3,300 points/s qd-sift-perf-0926: search covered 37% of the append; append 3,300 points/s search-during-write; no recall join |
...
| | sm-sift-perf-0926 | qd-sift-perf-0926 |
|---|--:|--:|
| storage on disk | 3.7 GiB | 2.9 GiB |
| peak RSS | 5.2 GiB | 5.2 GiB |
| disk read ops | 0 | 55,620 |
| disk write ops | 35,810 | 391,987 |
| disk read bytes | 0 | 2.0 GiB |
| disk write bytes | 2.7 GiB | 21.9 GiB |
| *syscall reads (all fds)* | *10,431,283* | *46,729* |
| *syscall writes (all fds)* | *4,699,554* | *7,742,579* |
...
| T3 ANN statistical equivalence | pass | strawmann n=10000 recall@1=0.9954 recall@10=0.9888 [0.9866,0.9907] recall@100=n/a MRDE=2.5537e-4 \| qdrant n=10000 recall@1=0.9940 recall@10=0.9876 [0.9852,0.9896] recall@100=n/a MRDE=3.2322e-4 \| recall@10 CIs overlap — QPS is comparable at this point |
...
| T4 quantization fidelity | pass | sq8/strawmann: \|Δscore\| over common ids n=98605 max=1.500e1 p99.9=1.130e0 p99=7.820e-1 p50=2.057e-1 mean=2.447e-1 (rel max=8.007e-2 p99=4.653e-3) (2790 ids not common to both lists) \| Kendall τ vs fp64 oracle = 0.9707 over 10000 queries (0 with <2 common ids excluded) \| quantization_dominance holds |
| T4 quantization fidelity | pass | sq8/qdrant: \|Δscore\| over common ids n=97270 max=1.217e1 p99.9=2.028e0 p99=1.593e0 p50=4.235e-1 mean=4.987e-1 (rel max=6.321e-2 p99=9.256e-3) (5460 ids not common to both lists) \| Kendall τ vs fp64 oracle = 0.9398 over 10000 queries (0 with <2 common ids excluded) \| quantization_dominance holds |

Reached **T4 quantization fidelity**; licenses performance and comparative claims. Conformance hash `e318626eac09f343`.

**sm-sift-perf-0926**: §7.1 gate pass, peak system load 25% of one core (all processes, this engine included), measured 2026-09-25. **qd-sift-perf-0926**: §7.1 gate pass, peak system load 34% of one core (all processes, this engine included), measured 2026-09-25.
<!-- END compare-full -->
