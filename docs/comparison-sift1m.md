...
|---|---|--:|--:|--:|--:|--:|--:|---|
| W0 | d=4 floor: graph traversal with the distance taken out | 4,452 | 3,272 | 1.36x | 221 µs / 269 µs | 305 µs / 373 µs | - | 1.36x is 1.93x less work per query x 0.65x cores busy during the row (0.69 against 1.06) x 1.08x clock: the middle term is occupancy, not search speed |
| W3 | search, fp32, single query | 1,748 | 1,936 | 0.90x | 587 µs / 705 µs | 519 µs / 722 µs | 0.9877 / 0.9873 |  |
| W4 | search, saturating (closed loop) | 23,477 | 10,501 | 2.24x | 2.73 ms / 3.06 ms | 6.13 ms / 7.06 ms | 0.9877 / 0.9873 | 2.24x is 1.57x less work per query x 1.37x cores busy during the row (7.53 against 5.48) x 1.04x clock: the middle term is occupancy, not search speed |
...
| W5 | search batched (16 distinct dataset queries per request) | 23,742 | 4,854 | 4.89x | 1.34 ms / 1.51 ms | 6.57 ms / 7.21 ms | 0.9877 / 0.9873 | per-batch latency (16 queries/request); queries: dataset, random-sample 4.89x is 1.35x less work per query x 3.58x cores busy during the row (7.05 against 1.97) x 1.01x clock: the middle term is occupancy, not search speed |
...
| W10-ef128 | recall control, ef=128 (latency only) | 20,860 | 9,853 | 2.12x | 385 µs / 479 µs | 798 µs / 1.30 ms | 0.9877 / 0.9873 | 2.12x is 1.57x less work per query x 1.31x cores busy during the row (6.72 against 5.14) x 1.03x clock: the middle term is occupancy, not search speed |
| W10-ef256 | recall control, ef=256 (latency only) | 12,272 | 5,699 | 2.15x | 658 µs / 879 µs | 1.41 ms / 2.14 ms | 0.9962 / 0.9969 | 2.15x is 1.44x less work per query x 1.47x cores busy during the row (7.01 against 4.77) x 1.02x clock: the middle term is occupancy, not search speed |
| W10-ef512 | recall control, ef=512 (latency only) | 6,748 | 3,148 | 2.14x | 1.19 ms / 1.70 ms | 2.55 ms / 3.75 ms | 0.9984 / 0.9994 | 2.14x is 1.34x less work per query x 1.57x cores busy during the row (7.08 against 4.50) x 1.02x clock: the middle term is occupancy, not search speed |
...
| W13 | scroll / pagination | 51,166 | 44,525 | 1.15x | 137 µs / 230 µs | 164 µs / 331 µs | - | sm-sift-perf-0923: -n 200000 (4x the table's 50000) qd-sift-perf-0923: -n 100000 (2x the table's 50000) sm-sift-perf-0923: 0.13 ms of the client's 0.14 ms p50 is not server time (12x), so 92% of this row is the load generator and the socket qd-sift-perf-0923: 0.13 ms of the client's 0.16 ms p50 is not server time (5x), so 81% of this row is the load generator and the socket 1.15x is 3.82x less work per query x 0.30x cores busy during the row (1.39 against 4.59) x 0.99x clock: the middle term is occupancy, not search speed |
