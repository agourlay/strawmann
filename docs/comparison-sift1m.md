...
|---|---|--:|--:|--:|--:|--:|--:|---|
| W0 | d=4 floor: graph traversal with the distance taken out | 4,052 | 3,368 | 1.20x | 245 µs / 302 µs | 296 µs / 370 µs | - |  |
| W3 | search, fp32, single query | 1,673 | 1,977 | 0.85x | 613 µs / 733 µs | 511 µs / 625 µs | 0.9873 / 0.9877 |  |
| W4 | search, saturating (closed loop) | 22,784 | 10,351 | 2.20x | 2.81 ms / 3.12 ms | 6.19 ms / 7.38 ms | 0.9873 / 0.9877 |  |
...
| W5 | search batched (16 distinct dataset queries per request) | 23,140 | 4,871 | 4.75x | 1.38 ms / 1.54 ms | 6.55 ms / 7.14 ms | 0.9873 / 0.9877 | per-batch latency (16 queries/request); queries: dataset, random-sample |
...
| W9 | exact / brute force | 191 | 144 | 1.33x | 37.23 ms / 69.42 ms | 54.59 ms / 75.08 ms | - |  |
...
| W10-ef128 | recall control, ef=128 (latency only) | 20,405 | 9,607 | 2.12x | 394 µs / 489 µs | 816 µs / 1.34 ms | 0.9873 / 0.9877 |  |
| W10-ef256 | recall control, ef=256 (latency only) | 11,925 | 5,573 | 2.14x | 677 µs / 900 µs | 1.43 ms / 2.19 ms | 0.9959 / 0.9970 |  |
| W10-ef512 | recall control, ef=512 (latency only) | 6,465 | 3,134 | 2.06x | 1.24 ms / 1.75 ms | 2.56 ms / 3.75 ms | 0.9980 / 0.9993 |  |
...
| W12-sel10-ef32 | filtered recall control, any of 10 keywords, ef=32 (latency only) | 382 | 4,614 | - | 5.26 ms / 6.06 ms | 428 µs / 549 µs | 0.9876 / 0.7962 | recall unequal: 0.9876 vs 0.7962; §7.4 compares at equal recall |
...
| W13 | scroll / pagination | 51,030 | 42,965 | 1.19x | 137 µs / 233 µs | 168 µs / 361 µs | - | sm-sift-perf-rel-0921: -n 200000 (4x the table's 50000) qd-sift-perf-rel-0921: -n 100000 (2x the table's 50000) |
| W11-steady | mixed read/write below the rebuild threshold: search bench2 while 50,000 synthetic points append | 17,859 | 4,444 | - | 446 µs / 577 µs | 1.69 ms / 4.78 ms | - | append 2,000 points/s search-during-write; no recall join |
| W11 | mixed read/write: search bench2 while 200,000 synthetic points append (runs last) | 1,794 | 1,773 | - | 3.42 ms / 13.51 ms | 3.76 ms / 14.48 ms | - | append 3,300 points/s search-during-write; no recall join |
